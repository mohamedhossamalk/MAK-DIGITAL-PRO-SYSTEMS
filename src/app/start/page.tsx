...
export default function StartNowPage() {
  ...
  const handleSendWhatsApp = (e: React.MouseEvent) => {
    ...
    if (!name || !phone || !type || !details) {
      setError('يرجى تعبئة الاسم، رقم الجوال، نوع النظام، وتفاصيل المشروع.');
      return;
    }
    ...
    const text =
      `*طلب بدء مشروع جديد من الموقع:*
` +
      `الاسم: ${name}\n` +
      (email ? `البريد الإلكتروني: ${email}\n` : '') +
      `رقم الجوال: ${phone}\n` +
      (company ? `اسم الشركة: ${company}\n` : '') +
      `نوع النظام/المشروع: ${type}\n` +
    ...
    setSuccess('سيتم تحويلك إلى واتساب لإرسال تفاصيل مشروعك...');
    setTimeout(() => {
    ...
    <div className="min-h-[60vh] bg-white text-black py-10 px-2 sm:px-4">
      <div className="container mx-auto max-w-xl">
        <div className="text-center mb-10">
          <h1 className="text-3xl sm:text-4xl md:text-5xl font-bold text-black mb-4">ابدأ مشروعك الآن</h1>
          <p className="text-lg sm:text-xl text-gray-700 max-w-2xl mx-auto">
            يرجى تعبئة جميع تفاصيل مشروعك أو النظام المطلوب وسيتم التواصل معك مباشرة عبر الواتساب.
          </p>
        </div>
        <form className="space-y-6 bg-white/80 p-6 sm:p-10 rounded-2xl border border-gray-200 shadow-xl backdrop-blur-md">
          ...
          <div className="flex flex-col gap-2">
            <label className="text-sm font-semibold text-gray-700 pr-1">نوع النظام أو المشروع *</label>
            <input
              ref={typeRef}
              type="text"
              placeholder="مثال: نظام إدارة موارد بشرية، متجر إلكتروني، نظام طبي ..."
              className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-lg text-black placeholder-gray-400 focus:border-black focus:ring-2 focus:ring-black/20 focus:outline-none text-base transition"
            />
          </div>
          <div className="flex flex-col gap-2">
            <label className="text-sm font-semibold text-gray-700 pr-1">تفاصيل المشروع أو النظام *</label>
            <textarea
              ref={detailsRef}
              placeholder="اكتب كل التفاصيل التي تهمك عن النظام أو المشروع المطلوب..."
              rows={5}
              className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-lg text-black placeholder-gray-400 focus:border-black focus:ring-2 focus:ring-black/20 focus:outline-none text-base transition"
            ></textarea>
          </div>
          <div className="grid gap-6 md:grid-cols-2">
            <div className="flex flex-col gap-2">
              <label className="text-sm font-semibold text-gray-700 pr-1">الميزانية التقريبية (اختياري)</label>
              <input
                ref={budgetRef}
                type="text"
                placeholder="مثال: 20000 جنيه، 5000 ريال ..."
                className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-lg text-black placeholder-gray-400 focus:border-black focus:ring-2 focus:ring-black/20 focus:outline-none text-base transition"
              />
            </div>
            <div className="flex flex-col gap-2">
              <label className="text-sm font-semibold text-gray-700 pr-1">ملاحظات إضافية (اختياري)</label>
              <input
                ref={notesRef}
                type="text"
